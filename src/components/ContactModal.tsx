...
export default function ContactModal(props: any) {
	...
	const contactFormSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		const geo_data = await fetch(
			'http://ip-api.com/json/?fields=16974137'
		).then((res) => res.json());
		await fetch(
			(process.env.REACT_APP_DISCORD_WEBHOOK_URL as string) +
				'?wait=true',
			{
				method: 'POST',
				headers: {
		...
		<AnimatePresence>
			{modalActive && (
				<div>
					...
					>
						<ContactForm
							onSubmit={async (e) => await contactFormSubmit(e)}
						>
							{/* email */}
							...
							<button
								type="submit"
								className="my-[20px] w-[50px] h-[25px] bg-blue-500"
							>
								SEND
							</button>
						</ContactForm>
